...
##            self.board_gui.updateTurn().capitalize() + "'s move.")
    ...
    def main(self):
        ...
        # Outside the loop so can still continue after wrong clicks
        # Should store the piece index 0, square index 1
        choice = []
            ...
            if click[1] == "square":
                ...
                for square in self.board_gui.squares:
                    ...
                    # Remove any invalid second clicks
                        ...
                        print("right here:", clicked_sqs)
                        if square.checkActive():
                            print("idk")                            
##                            # Make sure there are no duplicates
##                            if clicked_sqs.count(sq) >= 1:
##                                print("not alright")
##                                continue
##                            else:
##                                print("alright")
...
##                        clicked_sqs.remove(square)
                ...
                if clicked_sq == 1:
                    ...
                    # Check if it is on a valid piece 
                    if on_piece == False:
                        for sq in self.board_gui.squares:
                            # Check if there are any active sqs
                            if sq.checkActive():
                                print("here5")
                                active_sqs = True
                        # If there are no active squares, change
                            # the message
                        if not active_sqs:
                            print("place2")
                            self.board_gui.updateMessage(
                ...
                #### SECOND CLICK STUFF - AFTER VALID PIECE IS CHOSEN ####
                ...
                for sq in self.board_gui.getActiveSquares():
                    if sq.getLocation() == click_two[0].getLocation():
                        ...
                        valid_move = True
                        print("clicked_sqs update:", clicked_sqs)
                        # Make sure there are no duplicates
                        if clicked_sqs.count(sq) >= 1:
                            print("not alright")
                            continue
...
##                            break
                    # If not an active square, display error message
                        ...
                        print("error1111")
                        if not click_two[0].checkActive():
...
##                                    clicked_sqs.remove(click_two[0])
                                    ...
                                    rect.draw(self.board_gui.getWin())
                                    #self.board_gui.undrawPiece(piece)
                                    for sq in self.board_gui.getActiveSquares():
                                        sq.deactivate()

                                    # Reset validation lists and counters
                                    clicked_sqs = []
                                    clicked_sq = 0
                                    choice = []
                                # Sahil's stuff here?
                                ...
                                if spots != []:
                                    ...
                                    for spot in spots:
                                        for sq in self.board_gui.squares:
                                            ...
                                            if x == sq.getLocation()[0] and y == sq.getLocation()[1]:
                                                sq.activate()
                                    # Choice contains the valid piece object and square object
                                    choice.append(p)
            ...
            if clicktwo:
                ...
                if True:
                    ...
                    if valid_move: #and (p.checkPieceID == choice[0].checkPieceID):
                        ...
                        #move the piece
                        x,y = selectedPiece.getLocationXY()
                        enemyPieces = selectedPiece.movePiece(click[0].getLocation(),self.getEnemyTeam())
                        for piece in enemyPieces:
                            if piece.getEaten():
                        ...
                        print("Loc:", selectedPiece.getLocation())
                        self.board_gui.drawPiece(selectedPiece)
