        ...
        for sq in self.squares:
            ...
            if sq.clicked(pt) == True:
                ...
                if self.checkBoardStatus != 0:
                    ...
                    return "wrong sq"
                # If empty, update the message
                if sq.occupied == False:
                    # UPDATE THE MESSAGES IN CHESSGAME TO GET CORRECT TEAM
                    return "empty sq"
                else:
                    # check if the square's coords match a piece's coords
                    for piece in pieces:
                        for p in piece:
                            if sq.getLocation() == p.getLocation():
                                if p.checkColor == #team turn's color:
